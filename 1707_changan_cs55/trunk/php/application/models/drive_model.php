...
class Drive_model extends CI_Model {
    ...
    function __construct()
    {
        ...
        parent::__construct();
        $this->_tablename = $this->db->dbprefix('testdrive');
    }
    ...
    {
    	$sql = 'SELECT * FROM `' .$this->_tablename. '` WHERE prize IS NOT NULL ORDER BY `id` DESC LIMIT ?, ?';
    	$query = $this->db->query($sql, array(
    			$offset,
    			$length
    	));
	...
	{
		$sql = 'SELECT count(1) as c FROM `' .$this->_tablename. '` WHERE prize IS NOT NULL ';
    	$query = $this->db->query($sql);
    	$result = $query->result();
    	return $result[0]->c;
	}
     ...
     * @return array
     */
    public function getOneByRaffleCode($code)
    {
