	...
	this.init = function() {
		...
		this.loadprovince = function(e) {
			if(typeof e == 'string') {
				...
				config.carname = e.find("option").not(function() {
		...
		this.submitdrive = function() {
			...
			var proName = config.provinceEle.find("option").not(function() {
				return !this.selected
			}).text();
			var city = config.cityEle.val();
			var cityName = config.cityEle.find("option").not(function() {
				return !this.selected
			}).text();
			var town = config.townEle.val();
			var townName = config.townEle.find("option").not(function() {
				return !this.selected
			}).text();
			var dealerCode = config.dealerEle.val();
			var dealerName = config.dealerEle.find("option").not(function() {
				return !this.selected
			}).text();
			var dealerId = config.dealerEle.find('option').not(function() {
				return !this.selected
			}).attr("dealerid");
